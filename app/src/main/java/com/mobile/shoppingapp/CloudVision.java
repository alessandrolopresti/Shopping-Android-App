    ...
    private class LableDetectionTask extends AsyncTask<Object, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            if (activity != null && !activity.isFinishing()) {
                ...
                for (String item: res)  {
                    String[] realObject = item.split(":");
                    Button myButton = new Button(getApplicationContext());
                    float percentage = 0;
                    try {
                        percentage = Float.parseFloat(realObject[0]) * 100;
                    } catch (NumberFormatException e)   {
                        Log.d(TAG, "Cloud Vision API request failed " + e.getMessage());


                    }
                    myButton.setText(realObject[1] + " " + String.format("%.2f", percentage) + "%");
                    myButton.setOnClickListener(v -> {
                        SearchItem si = new SearchItem(realObject[1]);
                        ...
                        pushRef.child("Cart").push().setValue(si).addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                Toast.makeText(getApplicationContext(), realObject[1] + " has been successfully added to Your Cart", Toast.LENGTH_LONG).show();
                            }
                        }).addOnFailureListener(new OnFailureListener() {
                            @Override
                            public void onFailure(@NonNull Exception e) {
                                Toast.makeText(getApplicationContext(), realObject[1] + " could not be written", Toast.LENGTH_LONG).show();
                            }
                        });
                ...
                // ORA BISOGNA AGGIUNGERE L'onClick LISTENER SUI BOTTONI CREATI DINAMICAMENTE IN MODO DA POTER AGGIUNGERE
                // QUELL'OGGETTO ALLA LISTA DELLA SPESA
